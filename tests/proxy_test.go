...
func runGRPCProxyServerWithServerCount(serverCount int) (proxy, *server.ProxyServer, func(), error) {
	...
	server := server.NewProxyServer(uuid.New().String(), serverCount, &server.AgentTokenAuthenticationOptions{}, time.Hour)
...
func runHTTPConnProxyServer() (proxy, func(), error) {
	var proxy proxy
	s := server.NewProxyServer(uuid.New().String(), 0, &server.AgentTokenAuthenticationOptions{}, time.Hour)
	agentServer := grpc.NewServer()
