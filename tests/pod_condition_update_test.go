...
func TestPodConditionUpdate(t *testing.T) {
	...
	pod, err := k8sClient.CoreV1().Pods(pod.Namespace).Create(context.TODO(), pod, metav1.CreateOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	waitForTransition := func(target corev1.ConditionStatus, reason string) {
		...
		for i := 0; i < 100; i++ {
			...
			pod, err = k8sClient.CoreV1().Pods(pod.Namespace).Get(context.TODO(), pod.Name, metav1.GetOptions{})
			if err != nil {
				t.Fatal(err)
			}
