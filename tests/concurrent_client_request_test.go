...
func (s *singleTimeManager) AddBackend(agentID string, _ commonagent.IdentifierType, conn agent.AgentService_ConnectServer) server.Backend {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *singleTimeManager) RemoveBackend(agentID string, _ commonagent.IdentifierType, conn agent.AgentService_ConnectServer) {
	s.mu.Lock()
	defer s.mu.Unlock()
