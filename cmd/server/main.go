...
type ProxyRunOptions struct {
	...
	healthPort uint
	// After a duration of this time if the agent/master server doesn't see any activity it
	// pings the client to see if the transport is still alive.
	agentKeepaliveTime time.Duration
	masterKeepaliveTime time.Duration
...
func (o *ProxyRunOptions) Flags() *pflag.FlagSet {
	...
	flags.DurationVar(&o.agentKeepaliveTime, "keepalive-time", o.agentKeepaliveTime, "Time for gRPC server keepalive.")
	flags.DurationVar(&o.masterKeepaliveTime, "keepalive-time", o.masterKeepaliveTime, "Time for gRPC master keepalive.")
...
func (o *ProxyRunOptions) Print() {
	...
	klog.V(1).Infof("Agent keepalive time set to %v.\n", o.agentKeepaliveTime)
	klog.V(1).Infof("Master keepalive time set to %v.\n", o.masterKeepaliveTime)
...
func newProxyRunOptions() *ProxyRunOptions {
	o := ProxyRunOptions{
		...
		healthPort:                8092,
		adminPort:                 8095,
		agentKeepaliveTime:        1 * time.Hour,
		masterKeepaliveTime:       1 * time.Hour,
...
func (p *Proxy) runMTLSMasterServer(ctx context.Context, o *ProxyRunOptions, s *server.ProxyServer) (StopFunc, error) {
	...
	if o.mode == "grpc" {
		serverOptions := []grpc.ServerOption{
			...
			grpc.KeepaliveParams(keepalive.ServerParameters{Time: o.masterKeepaliveTime}),
		}
...
func (p *Proxy) runAgentServer(o *ProxyRunOptions, server *server.ProxyServer) error {
	...
	serverOptions := []grpc.ServerOption{
		grpc.Creds(credentials.NewTLS(tlsConfig)),
		grpc.KeepaliveParams(keepalive.ServerParameters{Time: o.agentKeepaliveTime}),
	}
	grpcServer := grpc.NewServer(serverOptions...)
...
>>>>>>> d0dadfc (updated to make flag names distinct)
