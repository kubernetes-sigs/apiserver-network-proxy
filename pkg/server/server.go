...
type ProxyServer struct {
	...
	connectionIDsByStreamIDMutex sync.Mutex

	idleTimerByStreamID      map[int64]*time.Timer
	idleTimerByStreamIDMutex sync.Mutex
}
...
func (s *ProxyServer) Proxy(stream client.ProxyService_ProxyServer) error {
	...
	if s.grpcMaxIdleTime != 0 {
		...
		// and keeping the map that stores the connection information clean.
		// Under certain conditions, it was noticed that apiserver establishes a connection by sending the DIAL_REQ
...
func (s *ProxyServer) serveRecvFrontend(stream client.ProxyService_ProxyServer, recvCh <-chan *client.Packet, streamID int64, t *time.Timer) {
	...
	for pkt := range recvCh {
		...
		case client.PacketType_CLOSE_REQ:
			...
			s.removeConnIDFromStreamIDMap(streamID, connID)
		...
		case client.PacketType_DATA:
			...
			// Reset timer as there is there is activity in the stream.
			s.resetTimer(t)

			// Store the connID in the streamID map.
			s.addConnIDToStreamIDMap(streamID, connID)
...
func (s *ProxyServer) resetTimer(t *time.Timer) {
	if s.grpcMaxIdleTime != 0 {
		if t != nil {
			if !t.Stop() {
				<-t.C
...
func (s *ProxyServer) addConnIDToStreamIDMap(streamID, connID int64) {
	if s.grpcMaxIdleTime != 0 {
		s.connectionIDsByStreamIDMutex.Lock()
...
func (s *ProxyServer) removeConnIDFromStreamIDMap(streamID, connID int64) {
	if s.grpcMaxIdleTime != 0 {
		s.connectionIDsByStreamIDMutex.Lock()
	...
	for pkt := range recvCh {
		...
		case client.PacketType_DIAL_RSP:
			...
			if frontend, ok := s.PendingDial.Get(resp.Random); !ok {
				...
				s.addConnIDToStreamIDMap(frontend.streamID, frontend.connectID)
			}
		...
		case client.PacketType_DATA:
			...
			// This is to reset the timer, in case there is no activity from the apiserver, but agent is sending
			// data to the apiserver.
			// Example: Watching the pod logs with kubectl -f option. In such a case after the initial DIAL_REQ and DATA
			// packet type from the apiserver but the agent keeps sending the DATA to the apiserver.
			// If the timer is not reset in this case, it leads to a closed connection after the timeout period, despite
